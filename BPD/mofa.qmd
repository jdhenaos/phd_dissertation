...
FCplotter = function(pathways, weights, dic, to.map, n.paths){
  ...
  for(i in 1:nrow(pathways)){
    ...
    pre.plot <- data.frame(
                       ...
                       ids=names(weights[names(weights) %in% gene_set])
                       )
    
    pre.plot <- pre.plot[!duplicated(pre.plot$ids),]
    pre.plot$Set_size <- nrow(pre.plot)
    
    to.plot <- rbind(to.plot, pre.plot)
  }
...
fc.f4 <- FCplotter(pathways = uni.f4[c(2,1,3,4),],weights = w.f4,dic=dic,to.map=or.f4,n.paths=5)
