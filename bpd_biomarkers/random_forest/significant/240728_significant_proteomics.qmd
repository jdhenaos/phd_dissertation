---
title: "Multi-class random forest using clinical and significant proteomics data"
author: Juan Henao
...
json.folder <- "/home/rstudio/project/input_data/random_forest/multiclass/significants/proteomics_json/"
...
ggsave(filename = "/home/rstudio/project/results/BPD/random_forest/multiclass/significant/proteomics/240728_rocauc.pdf",plot = p,
...
```{r}
stats <- rbind(
  data.frame(AUC=deep.omics$macro_mean_auc[1],
             std=deep.omics$macro_std_auc[1]),
  data.frame(AUC=deep.omics$macro_mean_auc[2],
             std=deep.omics$macro_std_auc[2]),
  data.frame(AUC=deep.omics$macro_mean_auc[3],
             std=deep.omics$macro_std_auc[3])
)
...
roc_data <- rbind(
  data.frame(Factor="Class 0",
             tpr=deep.omics$macro_tpr[[1]],
             fpr=deep.omics$macro_fpr[[1]]),
  data.frame(Factor="Class 1",
             tpr=deep.omics$macro_tpr[[2]],
             fpr=deep.omics$macro_fpr[[2]]),
  data.frame(Factor="CLass 2",
             tpr=deep.omics$macro_tpr[[3]],
             fpr=deep.omics$macro_fpr[[3]])
)
...
ggsave(filename = "/home/rstudio/project/results/BPD/random_forest/multiclass/significant/proteomics/240728_macro_rocauc.pdf",plot = p,
...
explanatory$Feature[1:6] <- c("Hospitalization [days]",
                                "KIR3DL2",
                              "Gestational age",
                              "NICU [days]",
                              "SCGF-beta",
...
p <- ggplot(explanatory[1:6,],aes(x=Importance,y=Feature,fill=Feature)) +
  ...
  labs(title = "Feature importance (random forest)",
       subtitle = "Deep clinical phenotyping +\nSignificant proteins (multi-class)") +
  theme(text = element_text(size = 8), plot.title = element_text(size=8)) + theme(legend.position = "none")

ggsave(filename = "/home/rstudio/project/results/BPD/random_forest/multiclass/significant/proteomics/240728_feature_importance.pdf",plot = p,
