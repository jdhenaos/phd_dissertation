...
AT.plot <- FCplotter(pathways = common.BP[1:8,],DEG = DEG.AT)
EC.plot <- FCplotter(pathways = common.BP[1:8,],DEG = DEG.EC)
MFB.plot <- FCplotter(pathways = common.BP[1:8,],DEG = DEG.MFB)
...
p <- ggplot(to.plot, aes(x = mean_FC, y = Cell, fill = interaction(Type,Cell) )) + ylab("Cell type") + xlab("Mean logFC") +
  ...
  theme(legend.direction = "vertical") + theme(strip.background =element_rect(fill="#4292C6")) + 
  theme(legend.position = "none", text = element_text(size=8), plot.title = element_text(size=8)) +
  geom_text(aes(label = Set_size, x =  ifelse(mean_FC < 0,-0.1, 0.1) ),size = 2.5) +
  geom_linerange(aes(xmin=ifelse(mean_FC < 0,mean_FC - SD, mean_FC), xmax=ifelse(mean_FC > 0,mean_FC + SD, mean_FC) )) + 
  coord_flip()

ggsave("output/figure1/common_genes.pdf",plot=p,device="pdf",units="mm",dpi=600,width = 160,height = 60)
```
...
to.plot$Cell <- factor(to.plot$Cell,levels = c("AT","FB"))
to.plot$Name <- as.factor(to.plot$Name)
...
ggsave("output/figure1/common_genes_at_fb.pdf",plot=p,device="pdf",units="mm",dpi=600,width = 160,height = 60)
```